...
def estimate_end_date(start: pd.Timestamp | None,
...
def build_forecast_df(licitaciones: pd.DataFrame,
    ...
    df["fecha_fin_explicit_dt"] = pd.to_datetime(
    ...
    df["inicio_efectivo"] = (df["fecha_inicio_dt"]
                              .fillna(df["fecha_adj_calc"])
                              .fillna(fpub))

    # Fin estimado
    df["fecha_fin_estimada"] = df.apply(
        lambda r: estimate_end_date(
            r["inicio_efectivo"], r["duracion_meses"],
            r["fecha_fin_explicit_dt"]), axis=1)

    # Ventana de re-licitación
    df["relicit_inicio"] = df["fecha_fin_estimada"] - pd.DateOffset(
        months=meses_anticipacion)
    ...
    df["estado_forecast"] = pd.cut(
        ...
        labels=["Ya vencido", "<3 meses", "3-6 meses",
                "6-12 meses", ">12 meses"],
    )
