...
def render(ctx: PageContext) -> None:
    ...
    if g.empty:
        ...
        c1, c2 = st.columns(2)
        with c1:
            fig = px.bar(g, x="mes", y="n", template=ctx.plotly_template,
        ...
        with c2:
            fig = px.area(g, x="mes", y="importe", template=ctx.plotly_template,
    ...
    if not df.empty and df["mes"].notna().any():
        ...
        pivot = hm.pivot(index="estado", columns="mes",
                          values="n").fillna(0)
        fig = px.imshow(pivot, aspect="auto", template=ctx.plotly_template,
